...
function virt-customize-wrapper() {
    command virt-customize -a /tmp/$ubuntu_distro-server-cloudimg-amd64.img "$@"
}
...
virt-customize-wrapper --install qemu-guest-agent
virt-customize-wrapper --run-command "echo -n > /etc/machine-id"
virt-customize-wrapper --run-command "apt-get update -y"
...
qm set $id --description "built on: $(date)"
qm template $id
